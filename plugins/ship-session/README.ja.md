...
スキルの `description` には日本語と英語の両方で起動例が書いてあるので、英語で頼んでも（`ship this` など）同じように起動します。
...
到達点の選択肢は PR作成まで（推奨） / マージまで / 実装まで（PRは作らない） / Issue作成まで の4つです（スキル本体では英語の正規ラベル `Up to PR (Recommended)` / `Up to merge` / `Implementation only (no PR)` / `Issue only` で定義され、ユーザーの言語に訳して表示されます）。
...
3. 到達点は次のどちらかで記録され、ゲートが開く
   - `AskUserQuestion` の header が `Goal` / `Approach`（英語）または `到達点` / `進め方`（日本語）なら、その回答を hook が**自動記録**する
   - それ以外の言語で聞いた場合や、ユーザーの発話に到達点が明示されている場合は、エージェントが `hooks/ship-goal.sh record "<到達点>"` を実行する。記録を忘れても、次にブロックされたツール呼び出しのメッセージが実行するよう伝える
...
スキル本体は英語で書かれていますが、**ユーザーに向けて出すものはユーザーの言語で書きます**。対象は `AskUserQuestion` の質問と選択肢、進捗や完了の報告、Artifact（タイトル・本文・ページ内の UI 文言）です。

言語は次の順に見て、最初に決まったものを使います。

1. Claude Code の `language` 設定（`~/.claude/settings.local.json` → `~/.claude/settings.json`）
2. ユーザーの直近の発話の言語
3. どちらでも決まらなければ英語

コード・コマンド・ファイルパスと、完了シグナルの接頭辞 `result:` / `needs input:` / `failed:` は訳しません（呼び出し側が文字どおりに探すため）。GitHub の Issue・PR・コミットメッセージはリポジトリの既存の慣習（既存の Issue/PR/コミットの言語、`CLAUDE.md` / `CONTRIBUTING.md` の規定）に従い、慣習が無いときだけユーザーの言語で書きます。
